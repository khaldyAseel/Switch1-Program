...
class Teacher(Person):
    ...
    def assign_course(self, course):
        # input validation1: Ensure course is not none
        if course is None:
            raise ValueError("Course cannot be None.")
        
        # input validation2: Ensure course is the correct type
        if not isinstance(course, Course):  
            raise TypeError(f"Should be Course instance, got {type(course).__name__}.")
...
class School:
    ...
    def get_students_with_failed_grades(self, fail_threshold=60):
        # to add input validation for fail_threshold
        failed_students = []
        for student in self.students:
