...
class AlreadyEnrolledError(Exception):
    """Raised when a student is already enrolled in the course."""
    pass

class NotEnrolledError(Exception):
    """Raised when a student is not enrolled in the course."""
    pass

class InvalidGradeError(Exception):
    """Raised when an invalid grade is assigned."""
    pass
    ...
    def assign_grade(self, course, grade):
        ...
        if course is None:
         ...
         if not isinstance(grade, (int, float)):
            raise InvalidGradeError(f"Invalid grade: {grade}. Must be a number.")
        if course in self.courses:
            self.grades[course].append(grade)
            print(f"Grade {grade} assigned to {self.name} for {course.name}")
        else:
            raise NotEnrolledError(f"{self.name} is not enrolled in {course.name}")
